(function() {
    ...
    function _InfoControl(map, projection, zoom, position) {
		_Control.call(this, map, 'avl-info-control', position);
        var self = this;
    ...
    function _ZoomControl(mapObj, map, zoom, position) {
		_Control.call(this, map, 'avl-zoom-control', position);
        var self = this,
        ...
        zoomButtons.append('div')
            .attr('class', 'avl-button avl-active avl-bold')
            .text('+')
            .on('click', function() {
                d3.event.stopPropagation();
                _clicked(1);
    ...
    function _LayerControl(mapObj, map, projection, zoom, position) {
		_Control.call(this, map, 'avl-layer-control', position);
        var self = this,
    ...
    function _MarkerControl(mapObj, map, projection, zoom, position) {
		_Control.call(this, map, 'avl-marker-control', position);
...
//IDtag, mapObj, projection, name, draggable)
    ...
    function _Map(IDtag, options, cntrls) {
        ...
        var layersDiv = map.append("div")
            .attr('id', 'avl-vector-layer')
            .attr("class", "avl-layersDiv");
        ...
        self.zoomMap = function() {
            ...
            if (rasterLayer) {
                var rTiles = rasterDiv
                    ...
                    .selectAll(".avl-r-tile")
                    .data(tiles, function(d) { return d; });

                rTiles.exit().remove();

                rTiles.enter().append('img')
                    .attr("class", 'avl-r-tile')
    ...
    avl.Map = function(options) {
        ...
        if (document.getElementById(id.slice(1)) === null) {
